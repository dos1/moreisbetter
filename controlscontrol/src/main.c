...
int main(int argc, char **argv){
	...
	al_set_app_name("Controls Control");
	...
	ALLEGRO_BITMAP *icon = al_load_bitmap(GetDataFilePath(&game, "icons/controlscontrol.png"));
	al_set_window_title(game.display, "Controls Control");
	...
	char libname[1024] = {};
	snprintf(libname, 1024, "libsuperderpy-%s-loading" LIBRARY_EXTENTION, "controlscontrol");
		...
		if (redraw && al_is_event_queue_empty(game._priv.event_queue)) {
			...
			while (tmp) {
				if ((tmp->pending_load) && (tmp->loaded)) {
					...
					char libname[1024];
					snprintf(libname, 1024, "libsuperderpy-%s-%s" LIBRARY_EXTENTION, "controlscontrol", tmp->name);
...
#define GS_ERROR FatalError(&game, false, "Error on resolving gamestate symbol: %s", dlerror()); tmp->pending_load = false; tmp->pending_start = false; tmp=tmp->next; continue;
			...
			if ((ev.type == ALLEGRO_EVENT_TIMER) && (ev.timer.source == game._priv.timer)) {
				...
				char filename[255] = { };
				snprintf(filename, 255, "ControlsControl_%ld_%ld.png", time(NULL), clock());
